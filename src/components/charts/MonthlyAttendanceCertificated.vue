...
export default {
	name    : 'monthly-attendance-chart-certificated',
	extends : BaseChart,
	...
	methods : {
		draw() {
			...
			const instructionalBarGroups = this.drawInstructionalBar();
		...
		drawInstructionalBar() {
			const instructionalBarHeight = 12;

			const instructionalBarGroups = this.canvas
				.selectAll( '.instructional-bar-groups' )
				.data( this.data )
				.enter()
				.append( 'g' );

			instructionalBarGroups.append( 'rect' )
				.attr( 'x', this.l + 1 )
				.attr( 'y', ( d, i ) => this.getBarY( i ) + this.barHeight )
				.attr( 'width', this.columnWidth * ( this.months.length / 2 ) )
				.attr( 'fill', 'transparent' )
				.attr( 'stroke', '#3F5356' )
				.attr( 'height', instructionalBarHeight );

			instructionalBarGroups
				.append( 'path' )
				.attr( 'd', ( d, i ) => {
					const { totalInstruction } = d;
					const fullWidth = this.columnWidth * ( this.months.length / 2 );
					const y = this.getBarY( i ) + this.barHeight + ( instructionalBarHeight / 2 );
					const x = this.l + 1;
					const endX = x + ( fullWidth * ( totalInstruction / 100 ) );


					return `M ${this.l + 1} ${y} L ${endX} ${y}`;
				} )
				.style( 'stroke', '#D0BA49' )
			...
			instructionalBarGroups
				.append( 'text' )
				.attr( 'x', this.l + 5 )
				.attr( 'y', ( d, i ) => this.getBarY( i ) + this.barHeight + ( instructionalBarHeight / 2 ) + 1 )
				.attr( 'dominant-baseline', 'middle' )
				.attr( 'text-anchor', 'start' )
				.attr( 'font-size', '10px' )
				.attr( 'font-weight', 'bold' )
				.text( d => `${Math.round( d.totalInstruction )}%` )
		...
		drawLeftLabels() {
			...
			const absenceNodes = teacherGroups
				...
				// ? Use this instead
				.attr( 'x', this.w - ( this.r + 80 ) )
				.attr( 'y', ( d, i ) => this.getLeftLabelY( i ) + this.barHeight )
				.attr( 'dominant-baseline', 'middle' )
